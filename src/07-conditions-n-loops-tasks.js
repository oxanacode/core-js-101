...
function findFirstSingleChar(str) {
  return str
    .split('')
    .filter((char) => [...str.matchAll(char)].length === 1 && char !== ' ')[0];
}
...
function isCreditCardNumber(ccn) {
  const cardNum = String(ccn).split('');
  let sum = 0;
  const remainder = cardNum.length % 2 === 0 ? 0 : 1;
  for (let i = 0; i <= cardNum.length - 1; i += 1) {
    const num = i % 2 === remainder ? +cardNum[i] * 2 : +cardNum[i];
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    else if (close.indexOf(str[i]) === open.indexOf(stack[stack.length - 1])) stack.pop();
    else return false;
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 1; i < arr.length; i += 1) {
    for (let j = 0; j < arr[i].length; j += 1) {
      if (arr[0][j] === arr[i][j]) intersection.push(arr[0][j]);
      else if (i > 1) {
        intersection.splice(j);
        break;
      }
    }
  }
